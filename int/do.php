...
$pgfn= function() use (&$v) {
    if (!empty(Settings::$req[0])) {
        ...
        if (! is_null($key)) {
            if ($rx = Settings::$sql->query("select user from userv u,sio_roleactivities r where r.role=u.role and r.activity=".Sec::CANPROVI." and school=".Settings::$usr['school']." order by name")) {
